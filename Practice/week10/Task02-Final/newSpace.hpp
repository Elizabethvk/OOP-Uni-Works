...
public:
    ...
    void saveBinary(string fileName) {
        ...
        if (file.is_open()) {
            ...
            std::cout<<"Created:\n";
            for (int i = 0; i < size; ++i) {
                std::cout<<i<<"."<<poletiN[i]->getYear()<<" "<<poletiN[i]->getName()<<"\n";
               // file.write((const char*)& poletiN[i], sizeof(poletiN[i]));
                file.write((const char*)& (*poletiN[i]), sizeof(Poleti));
                //file.write((const char*)& (*poletiN[i]), sizeof(poletiN[i]));
                //file << '\n';
               
            }
            std::cout<<"Adress of poletinN[0]:"<<& poletiN[0]<<std::endl;
            std::cout<<"ADress of Poleti object on place 0: "<<& (*poletiN[0])<<std::endl;
            std::cout<<"Size of poletinN[i]:"<<sizeof(poletiN[0])<<std::endl;
                std::cout<<"Size of class Poleti :"<<sizeof(Poleti)<<std::endl;
